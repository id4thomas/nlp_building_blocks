...
class AgentBuilder:
    ...
    @classmethod
    def _get_llm_client(cls) -> ChatOpenAI:
        return ChatOpenAI(
    ...
    @classmethod
    async def _get_location_tool(cls, user_id: str) -> List[StructuredTool]:
        mcp_client = MultiServerMCPClient(
            {
                "location": StreamableHttpConnection(
    ...
    @classmethod
    async def _get_weather_tool(cls) -> List[StructuredTool]:
        mcp_client = MultiServerMCPClient(
            {
                "weather": StreamableHttpConnection(
                    transport="streamable_http",
    ...
    async def build(
        ...
        if "location" in tool_names:
            location_tools = await cls._get_location_tool(user_id=user_id)
            tools.extend(location_tools)
        if "weather" in tool_names:
            weather_tools = await cls._get_weather_tool()
